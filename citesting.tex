...
$W_i = k(X_i,\cdot)(k(Y_i,\cdot) - k(Y_{i+1},\cdot))$
 ...
 \begin{lemma}
\label{lem:bnd}
Let $(X,X',E)$ be random variables such that  $|X-X'| \leq |E|$. For any  $\delta$
\[
\left | E  sgn(X) - sgn(X') \right | = O( P(|E|>\delta) + P(|X| < \delta))
\]
\end{lemma} 
\begin{proof}
Denote $A =|sgn(X) - sgn(X')|$ 
\begin{align*}
E A &= E( A | |E|>\delta) P(|E|>\delta) + E( A | |E|<\delta) P(|E|<\delta)  \\
&= O( P(|E|>\delta) ) +  E( A | |X|>\delta, |E|<\delta) P(|E|<\delta, |X|>\delta) \\
 & \quad \quad + E( A | |X|<\delta, |E|<\delta) P(|E|<\delta, |X|<\delta) \\
&= O( P(|E|>\delta) ) + O( P(|E|<\delta, |X|<\delta)) \\
& \leq O( P(|E|>\delta) + P(|X|<\delta))
 \end{align*}
\end{proof}
...
\paragraph{Distribution of distance between order stats}
\begin{lemma}
 Order statistics are independent
\end{lemma}
\begin{proof}
 Consider a set 
 \[
...
where $I_i < I_{i+1}$.
\[
 P(A_\alpha) = \prod_{i=1}^n P( (Z,Y,Z)_\alpha \in (I_i, B_i, C_i)) = \prod_{i=1}^n F(x_i,b_i,c_i) - F(x_i',b_i',c_i')
\]
so it works.
\end{proof}
reDefine 
